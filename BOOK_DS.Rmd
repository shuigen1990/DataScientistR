...
本书的写作对象是那些现在从事数据分析相关行业，或者之后想从事数据分析行业的人。本书意在为实践者提从事数据科学家这门职业的指导。读者可以从阅读中知道许多数据科学行业需要的技能，以及背后的“分析哲学”。书会选择部分最常用的，有效模型加以展开。

数据科学家这个行业的本质是应用。市面上有很多文章，出版物介绍各种数据模型，大多数此类书籍并不能让读者重复书中所述的分析过程，因此读者真正实践书中的知识会遇到很多困难。本书着重在于数据科学的实际应用，让读者能够重复书中的结果。本书极力避免数学公式和任何不像人话的语言，但可能有少数例外。我们在少数描述技术细节的初衷是为了帮助读者理解模型的长处和弱点，而非单纯的介绍数理统计知识。对于各种算法的实施，书中使用的是R语言，原因如下：

1. 免费的且可以在不同操作系统上使用。
1. 开源、可扩展：它在通用公共许可（General Public License）下发行，在此构架下任何人可以检查修改源程序。含有很多最新的模型。
1. The goal of R is "to turn ideas into software, quickly and faithfully" (John Chambers)
1. R有强大图形可视化和自动化报告功能
1. 笔者10年使用R的经验证明无论在学术还是业界，这都是非常有效的工具。

不熟悉R的读者可以在网上找到大量的入门教程。本书第2章-第4章主要介绍数据科学家这个行业和“分析哲学”和数据分析的一般流程。这是非技术的部分，但对于从业者来说非常重要，
它帮助你对这个职业设定一个合理的预期。其中回讨论数据科学家需要的技能。之后的章节会对其中部分我觉得重要的技能展开讨论，由于篇幅所限，不可能详细讨论开始这几章中提到的所有技能。
